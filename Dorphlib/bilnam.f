...
C     ------------------------------------------------------------------
      INTEGER*4 IWD(20),NAMFD(20),DSK(2),EXT(1)
C
C     This mess with iext(1) and inew(1)is to resolve a new warning
C     in gfortran-10 that about rank mismatch in the calls to LODUP and
C     LSNB.  
      INTEGER*4 IEXT(1),INEW(1)
C
      INTEGER*4 BLANK
C
...
C
      ...
      NAMFD(I)=BLANK
   10 CONTINUE
      IEXT(1)=BLANK                        !BLANK FIL/DEV EXT WORD
      INEW(1)=BLANK                        !BLANK NEW-FLAG
