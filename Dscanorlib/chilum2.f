C$PROG CHILUM2   - CHIL processor for L002 formatted data
C
C     ******************************************************************
C     BY J. A. Biggerstaff AT HHIRF - LAST MODIFIED by WTM 08/28/99
C     ******************************************************************
C
      SUBROUTINE CHILUM2(INBUF,NWDS)         !LINKAGE CHANGE * * *
C
      IMPLICIT NONE
C
C     ------------------------------------------------------------------
...
C
      INTEGER*4    HFWD
C
      PARAMETER   (HFWD=int(Z'FFFF'))             !HALFWORD MASK
C
...
C     ==================================================================
C
      IBPTR=0                              !INITIALIZE INPUT POINTER
C
C     ==================================================================
C     THE FOLLOWING CODE TELLS F7O THAT "ACCUM" NEEDN'T BE SAVED
C     AND CONTROLS WHEN THE TBUF POINTER IS SAVED.
C     ==================================================================
C
      ACCUM=0                              !KEEP F7O HAPPY
      TOFFSET=TOFSAVE                      !HELP F7O SOME MORE
C
      IF(TOFFSET.GE.0) GO TO 187           !CONTINUE UNPACKING
      TOFFSET=TOFFSET+1
      IF(TOFFSET.LT.0) GO TO 193           !RESYNCH REQUIRED
C
  179 CONTINUE                             !"SEARCH" FOR ID TAG
      IBPTR=IBPTR+1
      IF(IBPTR.GT.NWDS) GO TO 362          !END OF INPUT BUFFER
      IT=INBUF(IBPTR)                      !GET DATUM
      IF(IT.GE.-1)  GO TO 179              !NOT AN ID TAG
C
  184 CONTINUE                             !ID "FOUND"
      TOFFSET=IT-FFFF8001
      IF(TOFFSET.LT.0) GO TO 193           !DREAD '8000' ID
C
  187 CONTINUE                             !"UNPACK"
      IBPTR=IBPTR+1
      IF(IBPTR.GT.NWDS)  GO TO 362         !END OF INPUT BUFFER
      IT=INBUF(IBPTR)
      IF(IT.GE.0)  THEN                    !NOT A CODE WORD
       IF(TOFFSET.GE.NPAR)  THEN           !ID OUT OF RANGE
        DO 191 I=FWLEN,0,-1
        TBUFF(I)=-1                        !ZOT THE EVENT BUFFER
  191   CONTINUE
        GO TO 193                          !GO RESYNCH
       ENDIF
       TBUF(TOFFSET+1)=IT                                         !****
       TOFFSET=TOFFSET+1
       GO TO 187                           !KEEP ON TRUCKING
      ENDIF
      IF(IT.NE.-1) GO TO 184               !GO PROCESS ID TAG
      GO TO 217                            !'FFFF' EVENT COMPLETE
C
  193 CONTINUE                             !"RESYNCH" PROCEDURE
      TOFFSET=-1                           !FORCE "SEARCH" NEXT
  195 CONTINUE
      IBPTR=IBPTR+1
      IF(IBPTR.GT.NWDS)  THEN              !BUFF END
        TOFSAVE=-2                         !NOT YET SYNCHED
        RETURN
      ENDIF
      IF(INBUF(IBPTR).NE.-1)   GO TO 195   !KEEP LOOKING
      GO TO 179                            !SYNCHED, GO SEARCH
C
C     ==================================================================
C     Setup entry - deleted here - done in routine PVSETUP
C     ==================================================================
C
  362 TOFSAVE=TOFFSET                      !SAVE THE POINTER
      RETURN
C
C     ==================================================================
...
C
   26 CONTINUE                             !ADD-1 TO 16 BIT CHAN
      ...
      IF(RBN.EQ.-1) THEN                   !HALFWORD HIS
       CALL MEM_ADD1_HW(ACCUM+1)
      ELSE                                 !FULLWORD HIS
       CALL MEM_ADD1_FW(ACCUM/2+1)
      ENDIF
      GO TO 921
C
...
CX    ORD=ISHFT(ORD,-MILH(MILPTR+4))
      NSH=-MILH(MILPTR+4)                  !Make it I8*4
      ORD=ISHFT(ORD,NSH)
...
C
  239 CONTINUE                             !EVENT DONE
      DO 241 I=FWLEN,0,-1
      TBUFF(I)=-1                          !ZOT THE BUFFER
  241 CONTINUE
      TOFFSET=-1                           !IN CASE NO ID FOUND
      GO TO 179                            !GO SEARCH
C
...
C
 1011 CONTINUE                             !"PACK"
      ...
      MILPTR=MILPTR+2
      OOF=OOFSET(J)
      IF(OOF.LT.0)  STOP 'NO OUTPUT BUFFER'
      OEND=OOF+OSIZE(J)
      OOF=OOF+OPTR(J)
C
  133 CONTINUE                             !SCAN TBUF FOR PARAMS
      PARM=TBUF(ID1)                                              !****
      IF(PARM.LT.0) GO TO 144              !NULL PARM
      DATA=IOR(ID1,Z'FFFF8000')            !MUST PUSH ID TAG
      ASSIGN 138 TO RETPUSH                !IN-LINE SUBROUTINE CALL
      GO TO 159                            !GO PUSH
C
  138 DATA=PARM
      ASSIGN 141 TO RETPUSH
      GO TO 159                            !PUSH THE DATA
C
  141 ASSIGN 142 TO RETCHK
      GO TO 148                            !GO CHECK END
C
  142 PARM=TBUF(ID1)
      IF(PARM.GE.0) GO TO 138              !CONTIGUOUS PARAMETER
  144 ASSIGN 133 TO RETCHK                 !NULL PARAM
C
  148 CONTINUE                             !"ENDCHK"
      ID1=ID1+1
      IF(ID1.LE.IDL) GO TO RETCHK,(133,142)
      DATA=-1                              !MUST APPEND 'FFFF'
      ASSIGN 155 TO RETPUSH
      GO TO 159
C
  155 OPTR(J)=OOF-OOFSET(J)                !SAVE FOR LATER
      GO TO 221                            !END PACKING
C
  159 CONTINUE                             !"PUSH"
      OUTBUF(OOF)=DATA
      OOF=OOF+1
      IF(OOF.LT.OEND) GO TO RETPUSH,(138,141,155)
      OOFSET(J)=-1                         !FLAG NO BUFF
      CALL PUTPUT(J)                       !NOTE LINKAGE REVERSION
C
C     ------------------------------------------------------------------
C     FORTRAN DOES NOT SUPPORT "CO-ROUTINES" SO IT BECOMES THE
C     RESPONSIBILITY OF 'PUTPUT' TO SET:
C     OPTR(J)   = 0
C     OSIZE(J)  = APPROPRIATE SIZE
C     OOFSET(J) = APPROPRIATE OFFSET IN OUTBUF
C     ALL THESE THINGS HAVE BEEN PUT IN "COMMON/SC10/"
C     ------------------------------------------------------------------
C
      OOF=OOFSET(J)                        !HAS PVPINI BEEN DONE?
      IF(OOF.LT.0) STOP 'NO OUTPUT BUFFER'
      OEND=OOF+OSIZE(J)                    !OPTR(J) = 0 NOW
      GO TO RETPUSH,(138,141,155)
C
      END
