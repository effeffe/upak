...
C
      ...
      DIMENSION COMP(MXDAT,50),BETA(50),BERF(50),BERE(50)
      DIMENSION XHED(10)
C
      INTEGER*2 IUSE(500)
...
C
      ...
      EQUIVALENCE (XTSP(1),ITSP(1)),(BETA(1),B(1,1))
...
C
  620 IF(NF.GE.2) GO TO 630                   !Tst for ID entered
C
      IF(LASIDL.LE.0) GO TO 2020              !Tst for already defined
C
      IDN=LASIDL                              !If yes, set to saved ID
      GO TO 650                               !and go process
C
  630 CALL MILV(LWD(1,2),IDN,SV,KIND,IERR)
      IF(IERR.NE.0) GO TO 2010
      CALL GETDATX(IDN,IERR)
      IF(IERR.EQ.0) GO TO 650
      WRITE(6,640)IDN
  640 FORMAT(' DATA SET #',I6,'  NOT FOUND')
      LASIDL=-1                               !Set ID undefined
      GO TO 2500
C
C     ------------------------------------------------------------------
...
C
 ...
 1270 CONTINUE
      NLIN=101
C
      CALL PLOTFIT(1,XIN,YIN,YCAL,NDAT,XLIN,YLIN,NLIN)
C
...
C
 ...
 2025 FORMAT('Data ID number undefined - command not processed')
      GO TO 2400
C
 2050 IBAD='TERM'
      GO TO 2100
